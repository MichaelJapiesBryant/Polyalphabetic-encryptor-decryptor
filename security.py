...
class Security:
    ...
    def CaesarEncrypt(self, fileContent):
        ...
        for i in range(0, len(self.alphabet)):   #The actual length of the alphabet could be hardcoded here, however this enables for the alphabet to be changed without needing to change any of the code below
            letter = self.alphabet[i]
            shiftedLetter = self.alphabet[(i + userShift)%len(self.alphabet)]     #The modulo operator is used here to loop back around through the dictionary when the shift makes it go past the length. 
    ...
    def PolyEncrypt(self, fileContent):
        ...
        for letter in fileContent:    #For each letter in the text file, which has already been asigned to as 'fileContent'
            encryptedLetter = self.alphabet[(self.alphabet.index(letter) + self.alphabet.index(polyKey[i])) % len(self.alphabet)]   #The 'alphabet.index(polykey[i]) here acts the same as the userShift in the above Caesar function, 
    ...
    def CaesarDecrypt(self, Content):         #The Caesar decryption process is very similar to the encryption process, only that it does it backwards (Decrypts it). To do this in code it's fairly easy, as the shift just needs to be negative.
        ...
        for i in range(0, len(self.alphabet)):
            letter = self.alphabet[i]
            shiftedLetter = self.alphabet[(i + (-userShift))%len(self.alphabet)]      #This is where the only real change in the code is, the shift is simply reversed here. As long as the user remembers the shift they initially chose,
    ...
    def PolyDecrypt(self,Content):       #Just like with the Caesar decryption, this code is essentially the same as the encryption function, however just by  reversing the index of the passphrase/key
        i = 0                       
        encryptedText = '' 
        ...
        for letter in Content:      #are removed. However doing this would make the encrypted text less secure, so I have chosen to forego this requirement (As noted above it can be easily implemented if 100% neccisary)
            encryptedLetter = self.alphabet[(self.alphabet.index(letter) + -self.alphabet.index(polyKey[i])) % len(self.alphabet)]  #The only change is on this line, where the key is reversed ' -alphabet.index(polykey[i])
